...
public class ElementTextField extends ElementBase {
    ...
    public ElementTextField(
        GuiBase gui, int posX, int posY, int width, int height, short limit
    ) {
    ...
    public ElementTextField(
        GuiBase gui, int posX, int posY, String name, int width, int height, short limit
    ) {
        super(gui, posX, posY, width, height);
        setName(name);
        setPadding(1, 1, 1, 1);
    ...
    public ElementTextField
    setPadding(int paddingTop, int paddingRight, int paddingBottom, int paddingLeft) {
        this.paddingTop = paddingTop;
        this.paddingRight = paddingRight;
        this.paddingBottom = paddingBottom;
        this.paddingLeft = paddingLeft;
    ...
    public ElementTextField
    setSelectionColor(Number selectedLineColor, Number defaultCaretColor) {
    ...
    public ElementTextField
    setBackgroundColor(Number backgroundColor, Number disabledColor, Number borderColor) {
    ...
    protected void clearSelection() {
        if (selectionStart != selectionEnd) {
            if (selectionEnd < textLength) {
                System.arraycopy(
                    text, selectionEnd, text, selectionStart, textLength - selectionEnd
                );
            }
    ...
    protected int seekNextCaretLocation(int pos, boolean forward) {
        ...
        if (smartCaret) {
            for (int i = pos; i != e; i += dir) {
                char curChar = text[i];
                boolean caze
                    = Character.isUpperCase(curChar) != Character.isUpperCase(prevChar);
                if (caze
                    || Character.isSpaceChar(curChar) != Character.isSpaceChar(prevChar)
                    || Character.isLetterOrDigit(curChar)
                        != Character.isLetterOrDigit(prevChar)) {
                    if ((pos + dir) != i || !Character.isLetterOrDigit(curChar)) {
                        return i
                            + (smartCaretCase && caze && Character.isUpperCase(prevChar)
                                   ? -dir
                                   : 0);
                    }
                }
                ...
                switch (keyTyped) {
                        ...
                        if (!GuiScreen.isShiftKeyDown()) {
                            if (selectionStart != selectionEnd) {
                                ...
                                if (caret < textLength && textLength > 0) {
                                    --textLength;
                                    System.arraycopy(
                                        text, caret + 1, text, caret, textLength - caret
                                    );
                                }
                            }
                            if (caret <= renderStart) {
                    ...
                    case Keyboard.KEY_RIGHT: // right arrow
                        ...
                        if (GuiScreen.isShiftKeyDown()) {
                            ...
                            // the rest of the word the caret is on   i.e., a|bc|def ->
                            // a|bcdef| i don't know that it matters (home+end exhibit the
                            // former)
    ...
    public void drawBackground(int mouseX, int mouseY, float gameTicks) {
        drawModalRect(
            posX - 1, posY - 1, posX + sizeX + 1, posY + sizeY + 1, borderColor
        );
        drawModalRect(
            posX,
            posY,
    ...
    public void drawForeground(int mouseX, int mouseY) {
        ...
        FontRenderer font = getFontRenderer();
        char[] text = this.text;
        int startX = posX + paddingLeft, endX = sizeX - paddingRight,
            startY = posY + paddingTop, endY = startY + font.FONT_HEIGHT + paddingBottom;
        for (int i = renderStart, width = 0; i <= textLength; ++i) {
            ...
            if (drawCaret) {
                ...
                gui.drawSizedModalRect(
                    startX + width, startY - 1, startX + caretEnd, endY, -1
                );
