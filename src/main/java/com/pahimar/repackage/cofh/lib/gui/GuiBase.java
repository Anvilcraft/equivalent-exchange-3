...
/**
 * Base class for a modular GUIs. Works with Elements {@link ElementBase} and Tabs {@link
 * TabBase} which are both modular elements.
 *
 * @author King Lemming
 */
public abstract class GuiBase extends GuiContainer {
    public static final SoundHandler guiSoundManager
    ...
    public void handleMouseInput() {
        ...
        if (wheelMovement != 0) {
            for (int i = elements.size(); i-- > 0;) {
                ElementBase c = elements.get(i);
                if (!c.isVisible() || !c.isEnabled()
                    || !c.intersectsWith(mouseX, mouseY)) {
                    continue;
                }
    ...
    protected void mouseMovedOrUp(int mX, int mY, int mouseButton) {
        ...
        if (mouseButton >= 0 && mouseButton <= 2) { // 0:left, 1:right, 2: middle
            for (int i = elements.size(); i-- > 0;) {
                ElementBase c = elements.get(i);
                if (!c.isVisible()
                    || !c.isEnabled()) { // no bounds checking on mouseUp events
                    continue;
                }
    ...
    protected void mouseClickMove(int mX, int mY, int lastClick, long timeSinceClick) {
        ...
        if (this.field_147007_t && slot != null && itemstack != null
    ...
    public boolean isMouseOverSlot(Slot theSlot, int xCoord, int yCoord) {
        return this.func_146978_c(
            theSlot.xDisplayPosition, theSlot.yDisplayPosition, 16, 16, xCoord, yCoord
        );
    }
    ...
    /**
     * Abstract method to retrieve icons by name from a registry. You must override this
     * if you use any of the String methods below.
     */
    ...
    public void drawTiledTexture(int x, int y, IIcon icon, int width, int height) {
        ...
        for (i = 0; i < width; i += 16) {
            for (j = 0; j < height; j += 16) {
                ...
                drawScaledTexturedModelRectFromIcon(
    ...
    public void drawSizedTexturedModalRect(
        int x, int y, int u, int v, int width, int height, float texW, float texH
    ) {
        float texU = 1 / texW;
        float texV = 1 / texH;
        Tessellator tessellator = Tessellator.instance;
        ...
        tessellator.addVertexWithUV(
            x + 0, y + height, this.zLevel, (u + 0) * texU, (v + height) * texV
        );
        tessellator.addVertexWithUV(
            x + width, y + height, this.zLevel, (u + width) * texU, (v + height) * texV
        );
        tessellator.addVertexWithUV(
            x + width, y + 0, this.zLevel, (u + width) * texU, (v + 0) * texV
        );
        tessellator.addVertexWithUV(
            x + 0, y + 0, this.zLevel, (u + 0) * texU, (v + 0) * texV
        );
        tessellator.draw();
    }

    public void
    drawScaledTexturedModelRectFromIcon(int x, int y, IIcon icon, int width, int height) {
        if (icon == null) {
            return;
        }
        ...
        tessellator.startDrawingQuads();
        tessellator.addVertexWithUV(
            x + 0, y + height, this.zLevel, minU, minV + (maxV - minV) * height / 16F
        );
        tessellator.addVertexWithUV(
            x + width,
            y + height,
        ...
        tessellator.addVertexWithUV(
            x + width, y + 0, this.zLevel, minU + (maxU - minU) * width / 16F, minV
        );
    ...
    protected void drawTooltipHoveringText(List list, int x, int y, FontRenderer font) {
        ...
        this.drawGradientRect(
            i1 + k + 2, j1 - 3 + 1, i1 + k + 3, j1 + k1 + 3 - 1, i2, j2
        );
