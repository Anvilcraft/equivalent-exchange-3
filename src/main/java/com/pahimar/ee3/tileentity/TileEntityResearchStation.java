...
public class TileEntityResearchStation extends TileEntityEE implements IInventory {
    ...
    public String getInventoryName() {
        return this.hasCustomName() ? this.getCustomName()
                                    : Names.Containers.RESEARCH_STATION;
    }
    ...
    public boolean isItemValidForSlot(int slotIndex, ItemStack itemStack) {
        return slotIndex == ITEM_SLOT_INVENTORY_INDEX
            && BlacklistRegistryProxy.isLearnable(itemStack);
    }
    ...
    public Packet getDescriptionPacket() {
        return PacketHandler.INSTANCE.getPacketFrom(
            new MessageTileEntityResearchStation(this)
        );
    }
    ...
    public void updateEntity() {
        if (!this.worldObj.isRemote
            && inventory[ALCHENOMICON_SLOT_INVENTORY_INDEX] != null
    ...
    private boolean canLearnItemStack() {
        ...
        if (alchenomicon != null && playerName != null) {
            return PlayerKnowledgeRegistryProxy.canPlayerLearn(
                playerName, inventory[ITEM_SLOT_INVENTORY_INDEX]
            );
        }
    ...
    private boolean isItemStackKnown() {
        ...
        if (alchenomicon != null && playerName != null) {
            return PlayerKnowledgeRegistryProxy.doesPlayerKnow(
                playerName, inventory[ITEM_SLOT_INVENTORY_INDEX]
            );
        }
