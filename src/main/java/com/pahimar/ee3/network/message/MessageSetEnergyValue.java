...
public class MessageSetEnergyValue
    implements IMessage, IMessageHandler<MessageSetEnergyValue, IMessage> {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        if (compressedJsonLength != 0) {
            ...
            if (compressedValueMap != null) {
                ...
                try {
                    energyValueMap = SerializationHelper.GSON.fromJson(
                        jsonString, SerializationHelper.ENERGY_VALUE_MAP_TYPE
                    );
        ...
        if (energyValueMap != null) {
            byte[] compressedValueMap
                = CompressionHelper.compress(SerializationHelper.GSON.toJson(
                    energyValueMap, SerializationHelper.ENERGY_VALUE_MAP_TYPE
                ));
