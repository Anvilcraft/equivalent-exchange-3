...
public class MessageChalkSettings
    implements IMessage, IMessageHandler<MessageChalkSettings, IMessage> {
    ...
    public IMessage onMessage(MessageChalkSettings message, MessageContext ctx) {
        EquivalentExchange3.proxy.getClientProxy().chalkSettings
            = new ChalkSettings(message.index, message.size, message.rotation);
        return null;
    }
}