...
public class MessageSyncBlacklist
    implements IMessage, IMessageHandler<MessageSyncBlacklist, IMessage> {
        ...
        if (blacklist != null) {
            ...
            if (compressedJsonLength != 0) {
                ...
                if (compressedBlacklist != null) {
                    String jsonBlacklist
                        = CompressionHelper.decompress(compressedBlacklist);

                    try {
                        blacklistSet = SerializationHelper.GSON.fromJson(
                            jsonBlacklist, SerializationHelper.WRAPPED_STACK_SET_TYPE
                        );
                    } catch (JsonParseException e) {
                        LogHelper.warn(
                            "Failed to receive {} blacklist data from server", blacklist
                        );
                        blacklistSet = null;
                    }
                } else {
    ...
    public void toBytes(ByteBuf buf) {
        if (blacklist != null) {
            ...
            if (blacklistSet != null) {
                byte[] compressedBlacklist
                    = CompressionHelper.compress(SerializationHelper.GSON.toJson(
                        blacklistSet, SerializationHelper.WRAPPED_STACK_SET_TYPE
                    ));
