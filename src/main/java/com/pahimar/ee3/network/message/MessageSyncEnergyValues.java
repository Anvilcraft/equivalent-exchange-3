...
public class MessageSyncEnergyValues
    implements IMessage, IMessageHandler<MessageSyncEnergyValues, IMessage> {
    ...
    public void fromBytes(ByteBuf buf) {
        ...
        if (compressedJsonLength != 0) {
            ...
            if (compressedValueMap != null) {
                ...
                try {
                    valueMap = SerializationHelper.GSON.fromJson(
        ...
        if (valueMap != null) {
            byte[] compressedValueMap
                = CompressionHelper.compress(SerializationHelper.GSON.toJson(
                    valueMap, SerializationHelper.ENERGY_VALUE_MAP_TYPE
                ));
