...
public class MessageSliderElementUpdated
    implements IMessage, IMessageHandler<MessageSliderElementUpdated, IMessage> {
    ...
    public IMessage onMessage(MessageSliderElementUpdated message, MessageContext ctx) {
        ...
        if (entityPlayer != null) {
            if (entityPlayer.openContainer instanceof IElementSliderHandler) {
                ((IElementSliderHandler) entityPlayer.openContainer)
                    .handleElementSliderUpdate(message.elementName, message.elementValue);
            }
        }
