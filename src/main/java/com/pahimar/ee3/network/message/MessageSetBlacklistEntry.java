...
public class MessageSetBlacklistEntry
    implements IMessage, IMessageHandler<MessageSetBlacklistEntry, IMessage> {
    ...
    public MessageSetBlacklistEntry(
        Object object, Blacklist blacklist, boolean isBlacklistAction
    ) {
        ...
        if (blacklistOrdinal >= 0) {
            ...
            if (blacklist != null) {
                ...
                if (compressedJsonLength != 0) {
                    byte[] compressedWrappedStack
                        = buf.readBytes(compressedJsonLength).array();

                    if (compressedWrappedStack != null) {
                        String jsonWrappedStack
                            = CompressionHelper.decompress(compressedWrappedStack);

                        try {
                            wrappedStack
                                = SerializationHelper.GSON
                                      .fromJson(jsonWrappedStack, WrappedStack.class);
                        } catch (JsonParseException e) {
                            LogHelper.warn(
                                "Failed to receive {} blacklist data from server",
                                blacklist
                            );
                            wrappedStack = null;
                        }
    ...
    public void toBytes(ByteBuf buf) {
        if (blacklist != null) {
            ...
            if (wrappedStack != null && wrappedStack.getWrappedObject() != null) {
                byte[] compressedWrappedStack = CompressionHelper.compress(
                    SerializationHelper.GSON.toJson(wrappedStack)
                );
    ...
    public IMessage onMessage(MessageSetBlacklistEntry message, MessageContext ctx) {
        if (message.blacklist != null && message.wrappedStack != null) {
            if (message.isBlacklistAction) {
                BlacklistRegistryProxy.addToBlacklist(
                    message.wrappedStack, message.blacklist
                );
                BlacklistRegistry.INSTANCE.setShouldSave(false);
            } else {
                BlacklistRegistryProxy.removeFromBlacklist(
                    message.wrappedStack, message.blacklist
                );
                BlacklistRegistry.INSTANCE.setShouldSave(false);
            }
