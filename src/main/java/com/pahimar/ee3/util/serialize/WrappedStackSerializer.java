...
public class WrappedStackSerializer
    implements JsonSerializer<WrappedStack>, JsonDeserializer<WrappedStack> {
            ...
            if (jsonObject.has(TYPE_ITEM_STACK)) {
                try {
                    ItemStack itemStack = context.deserialize(
                        jsonObject.get(TYPE_ITEM_STACK), ItemStack.class
                    );
                ...
                try {
                    OreStack oreStack = context.deserialize(
                        jsonObject.get(TYPE_ORE_STACK), OreStack.class
                    );
                ...
                try {
                    FluidStack fluidStack = context.deserialize(
                        jsonObject.get(TYPE_FLUID_STACK), FluidStack.class
                    );
    ...
    @Override
    public JsonElement
    serialize(WrappedStack src, Type typeOfSrc, JsonSerializationContext context) {
        final JsonObject jsonObject = new JsonObject();

        if (src != null && src.getWrappedObject() != null) {
            jsonObject.add(
                src.getWrappedObject().getClass().getSimpleName().toLowerCase(),
                context.serialize(src.getWrappedObject())
            );
            return jsonObject;
        }
