...
public class ItemStackSerializer
    ...
    deserialize(JsonElement json, Type typeOfT, JsonDeserializationContext context)
        ...
        if (json.isJsonObject()) {
            ...
            if (jsonObject.has(META_VALUE)
                && jsonObject.get(META_VALUE).isJsonPrimitive()) {
                try {
            ...
            if (jsonObject.has(TAG_COMPOUND)
                ...
                try {
                    NBTBase nbtBase = JsonToNBT.func_150315_a(
                        jsonObject.getAsJsonPrimitive(TAG_COMPOUND).getAsString()
                    );
            ...
            if (name != null) {
                ...
                if (item != null) {
                    ItemStack itemStack = new ItemStack(
                        (Item) Item.itemRegistry.getObject(name), 1, metaValue
                    );
    ...
    @Override
    public JsonElement
    serialize(ItemStack src, Type typeOfSrc, JsonSerializationContext context) {
        if (src != null && src.getItem() != null) {
            JsonObject jsonObject = new JsonObject();

            if (Item.itemRegistry.getNameForObject(src.getItem()) != null) {
                jsonObject.addProperty(
                    NAME, Item.itemRegistry.getNameForObject(src.getItem())
                );
            } else {
