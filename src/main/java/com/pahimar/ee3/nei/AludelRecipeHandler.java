...
public class AludelRecipeHandler extends TemplateRecipeHandler {
    public class CachedAludelRecipe extends CachedRecipe {
        ...
        public CachedAludelRecipe(RecipeAludel recipe) {
            ...
            inputs = Arrays.asList(
                new PositionedStack(
                    new ItemStack(
                        ((ItemStack) wrappedInputs[0].getWrappedObject()).getItem(),
                        wrappedInputs[0].getStackSize(),
                        ((ItemStack) wrappedInputs[0].getWrappedObject()).getItemDamage()
                    ),
                    37,
                    7
                ),
                new PositionedStack(
                    new ItemStack(
                        ((ItemStack) wrappedInputs[1].getWrappedObject()).getItem(),
                        wrappedInputs[1].getStackSize(),
                        ((ItemStack) wrappedInputs[1].getWrappedObject()).getItemDamage()
                    ),
                    37,
                    28
                )
        ...
        public PositionedStack getOtherStack() {
            return new PositionedStack(
                FurnaceRecipeHandler.afuels
                    .get((cycleticks / 48) % FurnaceRecipeHandler.afuels.size())
                    .stack.item,
                37,
                63
            );
    ...
    public void loadTransferRects() {
        ...
        transferRects.add(
            new RecipeTransferRect(new Rectangle(74, 28, 24, 16), getRecipeID())
        );
    }
    ...
    public void loadUsageRecipes(ItemStack ingredient) {
        for (RecipeAludel recipe : AludelRecipeManager.getInstance().getRecipes()) {
            for (WrappedStack wrappedStack : recipe.getRecipeInputs()) {
                if (NEIServerUtils.areStacksSameTypeCrafting(
                        (ItemStack) wrappedStack.getWrappedObject(), ingredient
                    )) {
                    arecipes.add(new CachedAludelRecipe(recipe));
                }
            }
