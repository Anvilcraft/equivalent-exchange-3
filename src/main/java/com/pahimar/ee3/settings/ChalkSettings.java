...
public class ChalkSettings implements INBTTaggable {
    ...
    public void setIndex(int index) {
        ...
        if (this.index < 0) {
            ...
            this.index
                = AlchemyArrayRegistryProxy.getRegisteredAlchemyArrays().size() - 1;
        }
    }
    ...
    public void readFromNBT(NBTTagCompound nbtTagCompound) {
        if (nbtTagCompound != null && nbtTagCompound.hasKey("chalk_settings")
            ...
            NBTTagCompound chalkSettings
                = nbtTagCompound.getCompoundTag("chalk_settings");
            if (chalkSettings.hasKey("index")) {
                this.index = chalkSettings.getInteger("index");

                if (this.index < 0
                    || this.index
                        >= AlchemyArrayRegistryProxy.getRegisteredAlchemyArrays().size(
                        )) {
                    this.index = 0;
                }
            } else {
