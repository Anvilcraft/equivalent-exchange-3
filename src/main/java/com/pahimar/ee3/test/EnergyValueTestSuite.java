...
public class EnergyValueTestSuite {
    public static final Marker TEST_MARKER
        = MarkerManager.getMarker("EE3_ENERGY_VALUE_TEST", LogHelper.MOD_MARKER);
    private static final Marker SUCCESS_MARKER
        = MarkerManager.getMarker("EE3_TEST_SUCCESS", TEST_MARKER);
    private static final Marker FAILURE_MARKER
        = MarkerManager.getMarker("EE3_TEST_FAILURE", TEST_MARKER);
    private static final Marker FAILURE_WRONG_VALUE_MARKER
        = MarkerManager.getMarker("EE3_TEST_FAILURE_WRONG_VALUE", FAILURE_MARKER);
    private static final Marker FAILURE_NO_VALUE_MARKER
        = MarkerManager.getMarker("EE3_TEST_FAILURE_NO_VALUE", FAILURE_MARKER);
    ...
    public void run(boolean strict) {
        ...
        for (WrappedStack wrappedStack : testSuiteValueMap.keySet()) {
            ...
            EnergyValue actualValue
                = EnergyValueRegistryProxy.getEnergyValue(wrappedStack, strict);

            if (actualValue == null && expectedValue == null) {
                // Success - anticipated that no value was found and no value was found
                successMessages.add(String.format(
                    "SUCCESS: Object '%s' had the expected energy value [Expected (%s), Found (%s)]",
                    wrappedStack,
                    expectedValue,
                    actualValue
                ));
            } else if (actualValue == null) {
                // Failure - anticipated that a value would be found but no value was
                // found
                failureMessagesNoValue.add(String.format(
                    "FAILURE: Object '%s' did not have the expected energy value [Expected (%s), Found (%s)]",
                    wrappedStack,
                    expectedValue,
                    actualValue
                ));
                ...
                if (actualValue.equals(expectedValue)) {
                    //  Success - anticipated that a specific value would be found and the
                    //  anticipated value was found
                    successMessages.add(String.format(
                        "SUCCESS: Object '%s' had the expected energy value [Expected (%s), Found (%s)]",
                        wrappedStack,
                        expectedValue,
                    ...
                    // Failure - anticipated that a specific value would be found and
                    // while a value was found it was not the anticipated one
                    failureMessagesWrongValue.add(String.format(
                        "FAILURE: Object '%s' did not have the expected energy value [Expected (%s), Found (%s)]",
                        wrappedStack,
                        expectedValue,
                        actualValue
    ...
    public EnergyValueTestSuite load(File file) {
        try {
            ...
            LogHelper.warn(
                TEST_MARKER, "Could not load test file from disk: " + e.getMessage()
            );
            testSuiteValueMap = new TreeMap<>();
        }
