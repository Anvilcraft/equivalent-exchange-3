...
public class ItemChalk extends ItemEE implements IKeyBound {
    ...
    /**
     * Callback for item usage. If the item does something special on right clicking, he
     * will have one of those. Return True if something happen and false if it don't. This
     * is for ITEMS, not BLOCKS
     */
    @Override
    ...
    private boolean canPlaceAlchemyArray(
        ...
        if (!world.isRemote) {
            NBTTagCompound playerCustomData
                = EntityHelper.getCustomEntityData(entityPlayer);
            ...
            boolean canPlaceAlchemyArray
                = ModBlocks.alchemyArray.canPlaceBlockOnSide(world, x, y, z, side);

            int chargeLevel = ((chalkSettings.getSize() - 1) * 2) + 1;

            if (itemStack.getItemDamage() == itemStack.getMaxDamage()
                && (chargeLevel * chargeLevel) * alchemyArray.getChalkCostPerBlock()
                    == 1) {
                ...
                if (orientation == ForgeDirection.UP
                    ...
                    for (int i = x - coordOffset; i <= x + coordOffset; i++) {
                        for (int j = z - coordOffset; j <= z + coordOffset; j++) {
                            if ((i != x || j != z)
                                && (!ModBlocks.dummyArray.canPlaceBlockOnSide(
                                        world, i, y, j, side
                                    )
                                    || world.getTileEntity(i, y, j)
                                            instanceof TileEntityDummyArray)) {
                                canPlaceAlchemyArray = false;
                            }
                        }
                    }
    ...
    private void placeAlchemyArray(
        ItemStack itemStack,
        EntityPlayer entityPlayer,
        World world,
        int x,
        ...
        if (!world.isRemote) {
            ...
            placeBlockAt(
                entityPlayer, itemStack, world, x, y, z, ModBlocks.alchemyArray, side
            );
                        ...
                        for (int j = z - coordOffset; j <= z + coordOffset; j++) {
                            if (i != x || j != z) {
                                placeBlockAt(
                                    ...
                                    y,
                                    j,
                                    ModBlocks.dummyArray,
                                    side
                                );
                                if (world.getTileEntity(i, y, j)
                                        instanceof TileEntityDummyArray) {
                                    ((TileEntityDummyArray) world.getTileEntity(i, y, j))
                                        .setTrueCoords(x, y, z);
                                }
                            }
                        }
                    ...
                    for (int i = x - coordOffset; i <= x + coordOffset; i++) {
                        for (int j = y - coordOffset; j <= y + coordOffset; j++) {
                            if (i != x || j != y) {
                                ...
                                {
                                    ((TileEntityDummyArray) world.getTileEntity(i, j, z))
                                        .setTrueCoords(x, y, z);
                                }
                            }
                        }
                    ...
                    for (int i = y - coordOffset; i <= y + coordOffset; i++) {
                        for (int j = z - coordOffset; j <= z + coordOffset; j++) {
                            if (i != y || j != z) {
                                ...
                                {
                                    ((TileEntityDummyArray) world.getTileEntity(x, i, j))
                                        .setTrueCoords(x, y, z);
                                }
                            }
                        }
     ...
     *
     * @param itemStack    The item stack that was used to place the block. This can be
     *     changed inside the method.
     * @param entityPlayer The entityPlayer who is placing the block. Can be null if the
     *     block is not being placed by a entityPlayer.
     */
    private boolean placeBlockAt(
        EntityPlayer entityPlayer,
        ItemStack itemStack,
        World world,
        int x,
            ...
            if (world.getBlock(x, y, z) == block) {
                ...
                if (alchemyArray != null) {
                    itemStack.damageItem(
                        alchemyArray.getChalkCostPerBlock(), entityPlayer
                    );
    ...
    doKeyBindingAction(EntityPlayer entityPlayer, ItemStack itemStack, Key key) {
        if (key != Key.UNKNOWN) {
            ...
            PacketHandler.INSTANCE.sendTo(
                new MessageChalkSettings(chalkSettings), (EntityPlayerMP) entityPlayer
            );
        }
    }
}