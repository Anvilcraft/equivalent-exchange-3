...
public class ContainerAludel extends ContainerEE {
    ...
    public ContainerAludel(
        InventoryPlayer inventoryPlayer, TileEntityAludel tileEntityAludel
    ) {
        this.tileEntityAludel = tileEntityAludel;

        this.addSlotToContainer(
            new Slot(tileEntityAludel, TileEntityAludel.FUEL_INVENTORY_INDEX, 44, 74)
        );
        this.addSlotToContainer(
            new Slot(tileEntityAludel, TileEntityAludel.INPUT_INVENTORY_INDEX, 44, 18)
        );
        this.addSlotToContainer(
            new Slot(tileEntityAludel, TileEntityAludel.DUST_INVENTORY_INDEX, 44, 39)
        );
        this.addSlotToContainer(new Slot(
            tileEntityAludel, TileEntityAludel.OUTPUT_INVENTORY_INDEX, 120, 39
        ) {
            @Override
        ...
        for (int actionBarSlotIndex = 0; actionBarSlotIndex < PLAYER_INVENTORY_COLUMNS;
             ++actionBarSlotIndex) {
            this.addSlotToContainer(new Slot(
                inventoryPlayer, actionBarSlotIndex, 8 + actionBarSlotIndex * 18, 164
            ));
        }
    }
        ...
        for (Object crafter : this.crafters) {
            ...
            if (this.lastDeviceCookTime != this.tileEntityAludel.deviceCookTime) {
                icrafting.sendProgressBarUpdate(
                    this, 0, this.tileEntityAludel.deviceCookTime
                );
            }

            if (this.lastFuelBurnTime != this.tileEntityAludel.fuelBurnTime) {
                icrafting.sendProgressBarUpdate(
                    this, 1, this.tileEntityAludel.fuelBurnTime
                );
            }

            if (this.lastItemCookTime != this.tileEntityAludel.itemCookTime) {
                icrafting.sendProgressBarUpdate(
                    this, 2, this.tileEntityAludel.itemCookTime
                );
            }
        }
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < TileEntityAludel.INVENTORY_SIZE) {
                if (!this.mergeItemStack(
                        slotItemStack,
                        TileEntityAludel.INVENTORY_SIZE,
                        inventorySlots.size(),
                        false
                    )) {
                    return null;
                ...
                if (TileEntityFurnace.isItemFuel(slotItemStack)) {
                    if (!this.mergeItemStack(
                            slotItemStack,
                            TileEntityAludel.FUEL_INVENTORY_INDEX,
                            TileEntityAludel.OUTPUT_INVENTORY_INDEX,
                            false
                        )) {
                        return null;
                    }
                 ...
                 */
                    ...
                    if (!this.mergeItemStack(
                            slotItemStack,
                            TileEntityAludel.DUST_INVENTORY_INDEX,
                            TileEntityAludel.OUTPUT_INVENTORY_INDEX,
                            false
                        )) {
                        return null;
                 ...
                 */
                             ...
                             slotItemStack,
                             TileEntityAludel.INPUT_INVENTORY_INDEX,
                             TileEntityAludel.DUST_INVENTORY_INDEX,
                             false
                         )) {
                    return null;
