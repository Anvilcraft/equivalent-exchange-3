...
public class ContainerAlchemicalBag extends ContainerEE {
    ...
    public ContainerAlchemicalBag(
        ...
        for (int bagRowIndex = 0; bagRowIndex < bagInventoryRows; ++bagRowIndex) {
            for (int bagColumnIndex = 0; bagColumnIndex < bagInventoryColumns;
                 ++bagColumnIndex) {
                if (inventoryAlchemicalBag.parentItemStack.getItemDamage() == 0) {
                    this.addSlotToContainer(new SlotAlchemicalBag(
                        this,
    ...
    public void onContainerClosed(EntityPlayer entityPlayer) {
        ...
        if (!entityPlayer.worldObj.isRemote) {
            // We can probably do this better now considering the InventoryAlchemicalBag
            // has a findParent method
    ...
    public boolean isItemStackParent(ItemStack itemStack) {
        if (NBTHelper.hasUUID(itemStack)) {
            UUID stackUUID = new UUID(
                itemStack.getTagCompound().getLong(Names.NBT.UUID_MOST_SIG),
                itemStack.getTagCompound().getLong(Names.NBT.UUID_LEAST_SIG)
            );
    ...
    public ItemStack transferStackInSlot(EntityPlayer entityPlayer, int slotIndex) {
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            // Attempt to shift click something from the bag inventory into the player
            // inventory
            ...
            // Special case if we are dealing with an Alchemical Bag being shift clicked
                ...
                // Attempt to shift click a bag from the player inventory into the hot bar
                // inventory
                if (slotIndex < (bagInventoryRows * bagInventoryColumns)
                        + (PLAYER_INVENTORY_ROWS * PLAYER_INVENTORY_COLUMNS)) {
                    if (!this.mergeItemStack(
                            itemStack,
                            (bagInventoryRows * bagInventoryColumns)
                                + (PLAYER_INVENTORY_ROWS * PLAYER_INVENTORY_COLUMNS),
                ...
                // Attempt to shift click a bag from the hot bar inventory into the player
                // inventory
            ...
            // Attempt to shift click a non-Alchemical Bag into the bag inventory
                         ...
                         itemStack, 0, bagInventoryRows * bagInventoryColumns, false
                     )) {
                return null;
            }
    ...
    private class SlotAlchemicalBag extends Slot {
        ...
        public SlotAlchemicalBag(
            ContainerAlchemicalBag containerAlchemicalBag,
