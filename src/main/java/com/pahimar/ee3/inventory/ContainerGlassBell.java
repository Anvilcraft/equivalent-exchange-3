...
public class ContainerGlassBell extends ContainerEE {
    ...
    public ContainerGlassBell(
        InventoryPlayer inventoryPlayer, TileEntityGlassBell tileGlassBell
    ) {
        this.tileGlassBell = tileGlassBell;
        this.addSlotToContainer(new Slot(
            tileGlassBell, TileEntityGlassBell.DISPLAY_SLOT_INVENTORY_INDEX, 80, 26
        ));
        ...
        for (int actionBarSlotIndex = 0; actionBarSlotIndex < PLAYER_INVENTORY_COLUMNS;
             ++actionBarSlotIndex) {
            this.addSlotToContainer(new Slot(
                inventoryPlayer, actionBarSlotIndex, 8 + actionBarSlotIndex * 18, 137
            ));
        }
    }
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < TileEntityGlassBell.INVENTORY_SIZE) {
                ...
                if (!this.mergeItemStack(
                        slotItemStack, 0, TileEntityGlassBell.INVENTORY_SIZE, false
                    )) {
                    return null;
                }
            }
