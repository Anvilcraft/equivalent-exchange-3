...
public class ContainerAlchenomicon
    extends ContainerEE implements IElementButtonHandler, IElementTextFieldHandler {
    ...
    public ContainerAlchenomicon(EntityPlayer entityPlayer, ItemStack itemStack) {
        TreeSet<ItemStack> knownTransmutations
        ...
        if (itemStack.getItem() instanceof ItemAlchenomicon
            ...
            knownTransmutations.addAll(
                PlayerKnowledgeRegistryProxy.getKnownItemStacks(playerName)
            );
        }
        ...
        for (int rowIndex = 0; rowIndex < MAX_ROW_INDEX; ++rowIndex) {
            for (int columnIndex = 0; columnIndex < MAX_COLUMN_INDEX; ++columnIndex) {
                this.addSlotToContainer(new Slot(
                    inventoryAlchenomicon, i, 18 + columnIndex * 20, 18 + rowIndex * 19
                ) {
                    @Override
        ...
        for (int rowIndex = 0; rowIndex < MAX_ROW_INDEX; ++rowIndex) {
            for (int columnIndex = 0; columnIndex < MAX_COLUMN_INDEX; ++columnIndex) {
                this.addSlotToContainer(new Slot(
                    inventoryAlchenomicon, i, 140 + columnIndex * 20, 18 + rowIndex * 19
                ) {
                    @Override
                    public boolean canTakeStack(EntityPlayer player) {
                        return false;
    ...
    public void handleElementButtonClick(String elementName, int mouseButton) {
        if (elementName.equalsIgnoreCase("prev") && mouseButton == 0
            && this.pageOffset > 0) {
            this.pageOffset--;
            updateInventory();
    ...
    private void updateInventory() {
        ...
        List<ItemStack> filteredList = new ArrayList<>(FilterUtils.filterByDisplayName(
        ...
        if (pageOffset == 0) {
            ...
            newInventory = filteredList.subList(pageOffset * 80, (pageOffset + 1) * 80)
                               .toArray(newInventory);
            shouldUpdateInventory = true;
        } else if (pageOffset == maxPageOffset) {
            newInventory = filteredList.subList(pageOffset * 80, filteredList.size() - 1)
                               .toArray(newInventory);
            shouldUpdateInventory = true;
        }
