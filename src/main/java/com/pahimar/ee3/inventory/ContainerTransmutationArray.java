...
public class ContainerTransmutationArray extends ContainerEE {
    ...
    public ContainerTransmutationArray(
            ...
            for (int columnIndex = 0; columnIndex < maxArrayColumnCount; columnIndex++) {
                if (tileEntityAlchemyArray.getSize() == 1) {
                    this.addSlotToContainer(new Slot(
                        ...
                        columnIndex + rowIndex * maxArrayRowCount,
                        120 + columnIndex * 18,
                        69 + rowIndex * 18
                    ) {
                        @Override
        ...
        for (int inventoryRowIndex = 0; inventoryRowIndex < PLAYER_INVENTORY_ROWS;
            ...
            for (int inventoryColumnIndex = 0;
                ...
                this.addSlotToContainer(new Slot(
                    ...
                    47 + inventoryColumnIndex * 18,
                    173 + inventoryRowIndex * 18
                ));
            }
        }
        ...
        for (int actionBarSlotIndex = 0; actionBarSlotIndex < PLAYER_INVENTORY_COLUMNS;
             ++actionBarSlotIndex) {
            this.addSlotToContainer(new Slot(
                inventoryPlayer, actionBarSlotIndex, 47 + actionBarSlotIndex * 18, 231
            ));
        }
    }
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < inventorySize) {
                if (!this.mergeItemStack(
                        slotItemStack, inventorySize, inventorySlots.size(), false
                    )) {
                    return null;
                }
            } else {
