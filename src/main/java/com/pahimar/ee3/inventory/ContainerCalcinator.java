...
public class ContainerCalcinator extends ContainerEE {
    ...
    public ContainerCalcinator(
        ...
        this.addSlotToContainer(new Slot(
            tileEntityCalcinator, TileEntityCalcinator.FUEL_INVENTORY_INDEX, 45, 55
        ));

        // Add the input slot to the container
        this.addSlotToContainer(new Slot(
            tileEntityCalcinator, TileEntityCalcinator.INPUT_INVENTORY_INDEX, 45, 10
        ));

        // Add the output results slot to the container
        this.addSlotToContainer(new SlotCalcinator(
            tileEntityCalcinator,
            TileEntityCalcinator.OUTPUT_LEFT_INVENTORY_INDEX,
            105,
            29
        ));
        this.addSlotToContainer(new SlotCalcinator(
            tileEntityCalcinator,
            TileEntityCalcinator.OUTPUT_RIGHT_INVENTORY_INDEX,
            125,
            29
        ));
        ...
        for (int actionBarSlotIndex = 0; actionBarSlotIndex < 9; ++actionBarSlotIndex) {
            this.addSlotToContainer(new Slot(
                inventoryPlayer, actionBarSlotIndex, 8 + actionBarSlotIndex * 18, 152
            ));
        }
    }
    ...
    public void addCraftingToCrafters(ICrafting iCrafting) {
        ...
        iCrafting.sendProgressBarUpdate(
            this, 0, this.tileEntityCalcinator.deviceCookTime
        );
    ...
    public void detectAndSendChanges() {
        ...
        for (Object crafter : this.crafters) {
            ...
            if (this.lastBurnTime != this.tileEntityCalcinator.fuelBurnTime) {
                icrafting.sendProgressBarUpdate(
                    this, 1, this.tileEntityCalcinator.fuelBurnTime
                );
            }

            if (this.lastItemCookTime != this.tileEntityCalcinator.itemCookTime) {
                icrafting.sendProgressBarUpdate(
                    this, 2, this.tileEntityCalcinator.itemCookTime
                );
            }
        }
        ...
        if (slot != null && slot.getHasStack()) {
            ...
            if (slotIndex < TileEntityCalcinator.INVENTORY_SIZE) {
                if (!this.mergeItemStack(
                        slotItemStack,
                        TileEntityCalcinator.INVENTORY_SIZE,
                        inventorySlots.size(),
                        false
                    )) {
                    return null;
                ...
                if (TileEntityFurnace.isItemFuel(slotItemStack)) {
                    if (!this.mergeItemStack(
                            slotItemStack,
                            TileEntityCalcinator.FUEL_INVENTORY_INDEX,
                            TileEntityCalcinator.OUTPUT_LEFT_INVENTORY_INDEX,
                            false
                        )) {
                        return null;
                 ...
                 */
                             ...
                             slotItemStack,
                             TileEntityCalcinator.INPUT_INVENTORY_INDEX,
                             TileEntityCalcinator.OUTPUT_LEFT_INVENTORY_INDEX,
                             false
                         )) {
                    return null;
